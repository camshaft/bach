...
use bach::{
    environment::default::Runtime, ext::*, rand::Exhaustive, sync::queue::vec_deque::Queue,
};

fn run(f: impl Fn()) {
    crate::testing::init_tracing();

    let driver = Exhaustive::default();

    while driver.step().is_continue() {
        let mut rt = Runtime::new().with_coop(true).with_rand(driver.scope());
        rt.run(&f);
    }
}
